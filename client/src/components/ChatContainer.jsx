  ...
  return selectedUser? (
    ...
    <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messages.map((msg,index)=>(
          <div key={index} className={`w-full flex items-end gap-2 mb-2 ${msg.senderId === authUser._id ? 'justify-end' : 'justify-start'}`}>
...
/>
  ...
  <div className='flex-1 flex items-center bg-white/10 px-3 rounded-full'>
    <input onChange={(e)=>setInput(e.target.value)} value={input} onKeyDown={(e)=>e.key==="Enter" ?handleSendMessage(e):null}
      ...
      className='flex-1 bg-transparent text-sm p-3 border-none rounded-lg outline-none text-white placeholder-gray-400'
    />
